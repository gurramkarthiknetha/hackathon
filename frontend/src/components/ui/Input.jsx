const Input = ({ icon: Icon, error, ...props }) => {
	const hasError = !!error;

	return (
		<div className="mb-5">
			{/* Fixed height wrapper so icon stays aligned */}
			<div className="relative h-11">
				<div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
					<Icon
						className={`size-5 ${
							hasError ? "text-red-500" : "text-blue-500"
						}`}
					/>
				</div>
				<input
					{...props}
					className={`w-full h-full pl-10 pr-3 bg-gray-800 bg-opacity-50 rounded-lg border transition duration-200 text-white placeholder-gray-400 ${
						hasError
							? "border-red-500 focus:border-red-500 focus:ring-2 focus:ring-red-500"
							: "border-gray-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
					}`}
				/>
			</div>

			{/* Reserve space for error message */}
			<div className="mt-1 min-h-[1rem]">
				{hasError ? (
					<p className="text-xs text-red-500">{error}</p>
				) : (
					<p className="text-xs text-transparent select-none">.</p>
				)}
			</div>
		</div>
	);
};

export default Input;
